    ...
    try {
      ...
      if (formData.payment.method === 'card') {
        const { data } = await api.post('/api/payments/create-checkout-session', {
          ...
          shippingFeeCents,
          metadata: { source: 'checkout-page', cartItemsCount: items.length }
        });
        if (data?.url) window.location.assign(data.url);
        else console.error('URL de session Stripe manquante');
        ...
        if (data?.approveUrl) window.location.assign(data.approveUrl);
        else console.error('Lien d\'approbation PayPal manquant');
      }
    } catch (err) {
      ...
      <div className="checkout-header">
        ...
        <div className="checkout-steps">
          <div className={`step ${step >= 1 ? 'active' : ''}`}>
            <FiTruck /><span>Livraison</span>
          </div>
          <div className={`step ${step >= 2 ? 'active' : ''}`}>
            <FiCreditCard /><span>Paiement</span>
          </div>
          <div className={`step ${step >= 3 ? 'active' : ''}`}>
            <FiUser /><span>Confirmation</span>
          </div>
        </div>
      </div>
            ...
            {step === 1 && (
              ...
              >
                ...
                <div className="form-grid">
                  <input type="text" placeholder="Prénom" value={formData.shipping.firstName}
                    onChange={(e) => handleInputChange('shipping', 'firstName', e.target.value)} required />
                  <input type="text" placeholder="Nom" value={formData.shipping.lastName}
                    onChange={(e) => handleInputChange('shipping', 'lastName', e.target.value)} required />
                  <input type="email" placeholder="Email" value={formData.shipping.email}
                    onChange={(e) => handleInputChange('shipping', 'email', e.target.value)} required />
                  <input type="tel" placeholder="Téléphone" value={formData.shipping.phone}
                    onChange={(e) => handleInputChange('shipping', 'phone', e.target.value)} required />
                  <input type="text" placeholder="Adresse" className="full-width" value={formData.shipping.address}
                    onChange={(e) => handleInputChange('shipping', 'address', e.target.value)} required />
                  <input type="text" placeholder="Ville" value={formData.shipping.city}
                    onChange={(e) => handleInputChange('shipping', 'city', e.target.value)} required />
                  <input type="text" placeholder="Code postal" value={formData.shipping.postalCode}
                    onChange={(e) => handleInputChange('shipping', 'postalCode', e.target.value)} required />
                </div>
            ...
            {step === 2 && (
              ...
              >
                ...
                <div className="form-actions">
                  <button type="button" onClick={() => setStep(1)} className="back-btn">Retour</button>
                  {formData.payment.method === 'paypal' ? (
            ...
            {items.map(item => {
              ...
              return (
                <div key={item.id} className="order-item">
                  ...
                  {/* ── Aperçu personnalisation ou image simple ── */}
                  {hasCustom(item) ? (
                    <div className="order-item-custom-preview">
                  ...
                  <div className="item-info">
                    ...
                    {(item.color || item.size) && (
                      <p className="item-variant-detail">
                    ...
                    {/* Badge personnalisé */}
                    {hasCustom(item) && (
                      <span className="order-item-custom-badge">🎨 Personnalisé</span>
                    )}

                    {/* Détail prix si personnalisation */}
                    ...
                    {item.customization?.technique && (
                      <p className="item-technique">🖨 {item.customization.technique}</p>
                    )}
                  </div>
